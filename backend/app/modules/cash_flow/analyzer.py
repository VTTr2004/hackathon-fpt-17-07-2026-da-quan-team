...
class CashFlowAnalyzer:
    async def analyze(self, startup_facts: dict[str, Any], documents: list[dict[str, Any]], options: dict[str, Any]) -> ModuleReport:
        ...
        if metrics.get("net_burn", Decimal(0)) > 0: risks.append("MEDIUM — Dòng tiền hoạt động đang âm.")
        ...
        return ModuleReport(module=AnalysisModule.CASH_FLOW, version="1.0.0", status=status, score=score_data["total_score"], summary=f"Dòng tiền hoạt động {'âm' if metrics.get('net_burn') else 'tạo tiền'}; runway cơ sở {metrics.get('base_runway_months') or 'không áp dụng'} tháng.", findings=[Finding(title="Operating cash flow", detail=f"Net burn trung bình: {metrics.get('net_burn')} {dataset.currency}/tháng.", confidence="high"), Finding(title="Runway", detail=f"Runway cơ sở: {metrics.get('base_runway_months')} tháng.", confidence="high"), Finding(title="Reconciliation", detail=f"Trạng thái đối soát: {reconciliation['status']}.", confidence="high")], risks=risks, missing_data=[] if len(periods) >= 2 else ["At least two operating periods are required for reliable trend analysis."], assumptions=dataset.assumptions, recommended_questions=["Bổ sung working-capital balances để tính DSO/DPO và cash conversion cycle."], evidence=evidence, methodology=["Deterministic Decimal ledger calculation", "Financing inflows are excluded from operating burn"], tool_calls=calls, details={"cash_flow": {"currency": dataset.currency, "source_type": dataset.source_type}, "metrics": metrics, "periods": periods, "reconciliation": reconciliation, "scenarios": scenarios, "break_even": {"available": False, "reason": "Missing fixed costs or variable cost ratio."}, "working_capital": {"available": False, "missing_data": ["working_capital"]}, "matching_signals": matching, "warnings": [*dataset.warnings, *extraction_warnings, *duplicate_warnings]})
